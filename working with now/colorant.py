...
class Colorant:
    LOWER_COLOR = np.array([140, 90, 170])
    ...
    def listen(self):
        while True:
            ...
            if win32api.GetAsyncKeyState(0x02) < 0 and self.toggled:
                ...
                self.process("click")
            elif win32api.GetAsyncKeyState(0x56) < 0 and self.toggled:
                self.process("flick")
    ...
    def process(self, action):
        ...
        if action == "move":
            ...
            self.arduinomouse.flick(x_diff * self.movespeed, y_diff * self.movespeed)
